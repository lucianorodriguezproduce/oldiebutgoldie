...
const CompactSearchCard = memo(({ result, idx, onClick }: { result: DiscogsSearchResult, idx: number, onClick: () => void }) => {
    ...
    return (
        <motion.button
            ...
            onClick={onClick}
            className={`group relative flex flex-col items-start bg-white/[0.03] border ${isLocal ? 'border-primary/30 shadow-[0_0_30px_rgba(204,255,0,0.1)]' : 'border-white/5'} hover:border-primary/40 rounded-xl overflow-hidden transition-all text-left aspect-square md:aspect-[3/4] shadow-2xl`}
        >
