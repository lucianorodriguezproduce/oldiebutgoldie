...
export default function AlbumDetail() {
    ...
    if (error || !album) {
        return (
            ...
            >
                <p className="text-red-400 font-bold mb-8 text-xl tracking-tight">{TEXTS.comunidad.sonicConnectionLost}</p>
                <Link to="/">
                    <Button variant="outline" className="rounded-full px-10 h-14 font-bold border-white/20 hover:bg-white hover:text-black">{TEXTS.comunidad.backToDiscovery}</Button>
                </Link>
            </motion.div>
        );
    }
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Button
                                onClick={async () => {
                                    ...
                                    showLoading(isAdding ? TEXTS.comunidad.archivingCollection : TEXTS.comunidad.removingCollection);
                                    try {
                            ...
                            >
                                ...
                                <span className="relative z-10">{isInCollection(album.id.toString()) ? TEXTS.comunidad.archived : TEXTS.comunidad.collect}</span>
                            </Button>
                            <Button
                                onClick={async () => {
                                    if (!user) return alert(TEXTS.comunidad.syncToWantlist);
                                    const isAdding = !isInWantlist(album.id.toString());
                                    showLoading(isAdding ? TEXTS.comunidad.searchingFavorites : TEXTS.comunidad.removingFavorites);
                                    try {
                            ...
                            >
                                ...
                                {isInWantlist(album.id.toString()) ? TEXTS.comunidad.target : TEXTS.comunidad.favorites}
                            </Button>
                        </div>
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-10">
                            <h3 className="text-2xl font-display font-bold text-white flex items-center gap-4">
                                <Store className="h-7 w-7 text-primary" /> {TEXTS.comunidad.marketLogic}
                            </h3>
                        ...
                        <div className="space-y-8 font-mono">
                            <div className="flex justify-between items-end border-b border-white/[0.04] pb-6">
                                <span className="text-gray-500 text-xs font-bold uppercase tracking-widest">{TEXTS.comunidad.basePrice}</span>
                                <span className="text-3xl font-black text-white tracking-widest">
                                    {album.lowest_price ? `$${album.lowest_price.toFixed(2)}` : "—"}
                                </span>
                            </div>
                            <div className="flex justify-between items-end border-b border-white/[0.04] pb-6">
                                <span className="text-gray-500 text-xs font-bold uppercase tracking-widest">{TEXTS.comunidad.resonance}</span>
                                <div className="flex flex-col items-end">
                                    <span className="text-3xl font-black text-primary tracking-widest">
                                        {album.community?.rating?.average?.toFixed(1) || "5.0"}
                                    </span>
                                    <span className="text-[9px] text-gray-700 uppercase font-black mt-1">{album.community?.rating?.count || 0} {TEXTS.comunidad.communityVotes}</span>
                                </div>
                            </div>
                        </div>
                        <a href={album.uri} target="_blank" rel="noopener noreferrer" className="block w-full">
                            <Button className="w-full mt-10 bg-black/40 border border-white/10 text-white hover:bg-white hover:text-black transition-all rounded-[1.25rem] py-8 font-black uppercase tracking-widest text-[11px] h-16">
                                {TEXTS.comunidad.analyzeOnDiscogs}
                            </Button>
                        </a>
                    </motion.div>
                </div>
                ...
                <div className="lg:col-span-7 space-y-16">
                    ...
                    >
                        <div className="flex flex-wrap items-center gap-4 mb-8">
                            <div className="flex items-center gap-2 bg-secondary/10 px-5 py-2 rounded-full border border-secondary/20">
                                ...
                                <span className="text-[10px] font-black text-secondary uppercase tracking-[0.1em]">{album.formats?.[0]?.name || TEXTS.comunidad.lp}</span>
                            </div>
                            ...
                            {album.community?.have > 1000 && (
                                <div className="flex items-center gap-2 bg-primary/10 px-5 py-2 rounded-full border border-primary/20">
                                    ...
                                    <span className="text-[10px] font-black text-primary uppercase">{TEXTS.comunidad.eliteFavorite}</span>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 p-10 bg-white/[0.02] rounded-[3rem] border border-white/[0.04] backdrop-blur-2xl">
                            {[
                                { label: TEXTS.comunidad.label, value: album.labels?.[0]?.name },
                                { label: TEXTS.comunidad.catNo, value: album.labels?.[0]?.catno },
                                { label: TEXTS.comunidad.genre, value: album.genres?.[0] },
                                { label: TEXTS.comunidad.country, value: album.country },
                            ].map((item) => (
                                <div key={item.label}>
                    ...
                    >
                        {album.isBatch ? (
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center justify-between mb-10 border-b border-white/[0.05] pb-6">
                                    <h3 className="text-4xl font-display font-bold text-white tracking-tightest">{TEXTS.comunidad.tracklist}</h3>
                                    <span className="font-mono text-[11px] text-gray-700 uppercase font-black tracking-widest bg-white/5 px-4 py-2 rounded-xl">{album.tracklist?.length || 0} {TEXTS.comunidad.totalTracks}</span>
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pb-32">
                        <div className="bg-gradient-to-tr from-white/[0.02] to-transparent p-10 rounded-[3rem] border border-white/[0.04]">
                            <h4 className="text-gray-600 font-black mb-10 uppercase tracking-[0.25em] text-[10px]">{TEXTS.comunidad.sonicSpectrum}</h4>
                        ...
                        <div className="bg-gradient-to-tr from-white/[0.02] to-transparent p-10 rounded-[3rem] border border-white/[0.04]">
                            <h4 className="text-gray-600 font-black mb-10 uppercase tracking-[0.25em] text-[10px]">{TEXTS.comunidad.ownershipData}</h4>
                            <div className="grid grid-cols-2 gap-8">
                                <div className="flex flex-col">
                                    <span className="text-5xl font-black text-white tracking-widest">{album.community?.have || 0}</span>
                                    <span className="text-[10px] uppercase font-black tracking-[0.15em] text-gray-500 mt-3">{TEXTS.comunidad.collectors}</span>
                                </div>
                                <div className="flex flex-col">
                                    <span className="text-5xl font-black text-secondary tracking-widest">{album.community?.want || 0}</span>
                                    <span className="text-[10px] uppercase font-black tracking-[0.15em] text-gray-500 mt-3">{TEXTS.comunidad.wanted}</span>
                                </div>
                            </div>
                        </div>
                    </div>
