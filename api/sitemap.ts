...
async function initBunkerIdentity() {
    ...
    const serviceAccount = JSON.parse(payload); // <--- OBLIGATORIO: Convertir string a objeto

    if (!admin.apps.length) {
        admin.initializeApp({
            credential: admin.credential.cert(serviceAccount) // <--- PASAR EL OBJETO, NO EL STRING
        });
